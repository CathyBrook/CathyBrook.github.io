---
layout: page
title: two-energy XAS
description: This is one of my favorate science work that is able to measure two XAS simultaneously using a lab source! But it's too niche, so actually got almost no citations. 
img: assets/img/gallery/gallery_two_E_XAS.png
importance: 2
category: work
giscus_comments: false
---

The website is still in progress. Project details will be added soon.

<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/gallery/gallery_two_E_XAS.png" title="" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    two E XAS
</div>
